...
status: active
updated: 2026-06-18
description: "관리자 콘솔 Phase II — 결제 관리 (Feature 2) 설계 (Design Spec) 상세 요구사항 및 기능 동작 명세서"
...
| POST | `/api/admin/payments/{id}/refund` | 관리자 강제 환불 | 400 (상태 가드 / 토스 실패 — 기존 `GlobalExceptionHandler` 의 `PaymentFailedException` 매핑 재사용) |
...
2. 가드:
   - payment.status != CONFIRMED → `PaymentFailedException` (기존) → 400 "승인된 결제만 환불할 수 있습니다"
...
3. 토스 호출 (tossCancelProperties.enabled 가 true 이고 paymentKey != null 일 때):
   ...
   - 실패 시 `PaymentFailedException` → 400 (기존 매핑 — 트랜잭션 롤백)
   - paymentKey == null 이고 enabled == true 이면 `PaymentFailedException` → 400 "환불을 위한 결제키가 없습니다"
4. Payment 상태 전이:
...
두 트랜잭션이 모두 status=CONFIRMED 를 읽고 진행 가능. 토스 `cancel` API 는 paymentKey 기준 idempotent — 두 번째 호출은 `ALREADY_CANCELED_PAYMENT` 4xx 응답 → `PaymentFailedException` → 400 (기존 매핑, 트랜잭션 롤백). DB 는 첫 번째 커밋만 반영되어 최종 일관성 유지. 본 MVP 에서는 application-level 직렬화(@Version/SELECT FOR UPDATE) 불필요.

### 6.2 토스 API 실패 시나리오

- 4xx (이미 취소된 paymentKey 등): `PaymentFailedException` → 400 (기존 매핑). 트랜잭션 롤백으로 Payment 유지, 관리자 재시도 가능
...
- 시드/마이그레이션 잔재 데이터
- `toss-cancel-enabled=true` + paymentKey NULL → `PaymentFailedException` → 400 "환불을 위한 결제키가 없습니다"
...
- PENDING 환불 시도 → `PaymentFailedException` (400)
- CANCELLED 재환불 시도 → `PaymentFailedException` (400, 중복 환불은 status 가드로 차단)
- FAILED 환불 시도 → `PaymentFailedException` (400)
