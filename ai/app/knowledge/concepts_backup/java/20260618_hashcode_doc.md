...
## 핵심 설명
hashCode는 객체를 HashMap·HashSet 같은 해시 기반 컬렉션에서 어떤 버킷(bucket)에 넣고 찾을지 빠르게 정하기 위해 정수 해시값을 반환하는 메서드다.
...
## 흔한 오해
- hashCode가 객체의 메모리 주소를 그대로 리턴한다고 오해하기 쉽다. 사실 기본 구현값도 메모리 주소가 아니라 JVM이 내부적으로 정한 방식으로 생성한 정수이고, 재정의하면 개발자가 고른 핵심 필드로부터 계산한 값을 반환한다.
