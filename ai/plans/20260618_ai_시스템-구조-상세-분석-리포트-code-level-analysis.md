...
status: active
updated: 2026-06-18
description: "AI 시스템 구조 상세 분석 리포트 (Code-level Analysis) 도입 계획 및 마이그레이션 작업 방향"
...
- 🔴 **타임아웃 (`readTimeoutSeconds: 0`)**: RestClient 설정상 0은 **무한 대기(Infinite Wait)**를 의미합니다. AI 응답 지연 시 Spring Tomcat thread pool이 고갈되어 전체 서비스 장애로 번질 수 있으므로 **반드시 25~35초(기본 30초 권장)**로 변경해야 합니다.
...
- `readTimeoutSeconds`를 0 → 30으로 변경 (권장 범위 25~35초)
...
- [ ] **타임아웃 수정** (Highest Priority)  
      `application.yml`의 `python.read-timeout-seconds`와 `ollama.read-timeout-seconds`를 `0` → `30`으로 변경. 운영 권장 범위는 `25~35초`입니다.
      _이유: Spring → Python → Ollama blocking call chain에서 Tomcat thread pool 고갈 방지_
...
| **Lifecycle state event rate** | 미측정 | 100% | IDLE/LOADING/STREAMING/SUCCESS/TIMEOUT/RETRYING/FALLBACK/ERROR 이벤트 수집률 |
| **Hallucination rate** | 미측정 | 2% 이하 | RAG contradiction, unsupported claim, topic relevance downgrade |

> 🔴 **최우선 위험**: `readTimeoutSeconds: 0`(무한 대기)은 "느린 AI 요청을 오래 기다린다" 수준이 아니라, Spring Tomcat thread pool을 무기한 점유해 전체 서비스 장애로 번질 수 있는 설정입니다. timeout이 없으면 timeout 지표도 수집되지 않으므로 운영자는 장애를 "AI timeout"이 아니라 "전체 API가 멈춘 것처럼 보이는 현상"으로 보게 됩니다. ✅ 권장값은 `25~35초`이며, 기본 운영값은 `30초`를 권장합니다.
...
- 🔴 **`readTimeoutSeconds: 0` 위험**: Python 또는 Ollama가 멈추면 Spring worker thread가 반환되지 않습니다. 요청이 누적되면 로그인, 커뮤니티, 학습 결과 조회처럼 AI와 무관한 API도 같은 Tomcat pool에서 밀립니다.
- ✅ **추천값**: `python.read-timeout-seconds=25~35`, `ollama.read-timeout-seconds=25~35`. 25초 미만은 로컬 CPU Ollama cold start에서 fallback이 과도할 수 있고, 45초 이상은 사용자가 이미 장애로 체감할 가능성이 큽니다.
...
Blocking call chain은 아래처럼 이어집니다.

```mermaid
flowchart LR
    FE["Frontend request"] --> SPRING["Spring MVC / Tomcat worker"]
    SPRING -->|"RestClient blocking call"| PY["Python FastAPI worker"]
    PY -->|"urllib blocking call"| OLLAMA["Ollama generate queue"]
    OLLAMA --> CPU["CPU inference"]
    CPU --> OLLAMA --> PY --> SPRING --> FE

    SPRING -. "readTimeoutSeconds: 0이면 무한 점유" .-> RISK["🔴 Tomcat thread pool exhaustion"]
    PY -. "Ollama 지연/다운이면 대기" .-> RISK
```
...
### 개선 방향
- ✅ 단기: Spring MVC는 유지하되 `@Async` + `CompletableFuture`, SSE, job queue + polling 중 하나로 AI 호출을 분리합니다. 목표는 생성 완료까지 요청 thread를 붙잡지 않는 것입니다.
- ✅ 중기: WebFlux/Reactor 전환은 전체 서비스가 아니라 Python AI client adapter 경계부터 부분 적용합니다.
- ✅ Streaming UX: 실제 처리량은 그대로여도 first-token latency를 낮추고, 사용자가 "멈춘 화면"이 아니라 진행 중인 답변을 보게 합니다.
...
## 23.1 readTimeoutSeconds: 0 운영 위험과 권장 정책

`readTimeoutSeconds: 0`은 AI 기능만 느려지는 문제가 아닙니다. 현재 호출 구조가 `Frontend → Spring → Python → Ollama`로 이어지는 blocking chain이기 때문에, 하위 계층이 멈추면 상위 계층의 worker도 같이 묶입니다.

🔴 **위험 요약**

- Spring `RestClient`가 Python 응답을 무한 대기합니다.
- Tomcat worker thread가 반환되지 않습니다.
- 동시 AI 요청이 누적되면 thread pool이 고갈됩니다.
- AI와 무관한 API도 Tomcat queue에서 밀립니다.
- 사용자는 "AI 답변이 느림"이 아니라 "서비스 전체가 멈춤"으로 체감합니다.
...
ai-review:
  python:
    read-timeout-seconds: 30   # 권장 범위: 25~35
  ollama:
    read-timeout-seconds: 30   # 권장 범위: 25~35
```

- `25초`: 빠른 fallback과 thread 보호에 유리합니다.
- `30초`: 로컬 CPU Ollama cold start와 짧은 답변 생성을 균형 있게 허용합니다.
- `35초`: 느린 노트북 환경에서 fallback 과다 발생을 줄이되, 사용자 이탈 전 상한을 둡니다.
- `45초 이상`: 운영 UX 관점에서는 과도합니다. 사용자가 기다림을 장애로 인식하기 쉽습니다.
- `0초`: 금지에 가깝습니다. timeout metric도 잡히지 않아 장애 분석이 어려워집니다.
...
sequenceDiagram
    ...
    FE->>T: POST /messages
    T->>PY: RestClient blocking call
    PY->>OL: generate blocking call
    alt readTimeoutSeconds = 0
        OL--xPY: delayed or stuck
        PY--xT: no bounded response
        Note over T: 🔴 Thread held indefinitely
        Note over FE: User sees endless loading
    else readTimeoutSeconds = 25~35
        OL--xPY: timeout/error
        PY-->>T: timeout or fallback
        T-->>FE: fallback response / retry state
    end
```

## 23.2 Retry / Circuit Breaker / Fallback 정책 구체화

현재 문서의 "Circuit Breaker + Retry + Fallback"은 방향성으로는 맞지만, 운영에서는 retry 조건과 횟수를 명확히 제한해야 합니다. AI 생성 요청은 비용이 크고 느리기 때문에 retry를 많이 할수록 장애가 더 커질 수 있습니다.

✅ **권장 Retry 정책**

- Retry 방식: **Exponential backoff + Full Jitter**
- 최대 retry: **2회**, 많아도 **3회**
- 3회 실패 시 즉시 fallback
- retry 간격 예시:
  - 1차 실패 후: `200~800ms` 랜덤
  - 2차 실패 후: `800~2000ms` 랜덤
  - 3차 실패 후: fallback
- timeout 상한: retry 전체 시간을 포함해 사용자 체감 `25~35초` 안에 끝나야 합니다.

```text
delay = random(0, base * 2^attempt)
attempt=1: random(0~800ms)
attempt=2: random(0~1600ms)
attempt=3: fallback
```
...
- Python AI server 일시 과부하
- Ollama cold start 중 timeout

🔴 **Non-retryable Exception**

- HTTP 400 / 422: request schema 오류
- HTTP 401 / 403: service token 또는 권한 오류
- prompt/input validation 실패
- 모델 이름 오류, 설정 오류
- content policy / guardrail reject

✅ **Idempotency 고려**

- follow-up/free-question 생성은 DB write보다 "생성 결과 조회"에 가까워 retry가 가능하지만, candidate capture나 message 저장은 중복 저장 위험이 있습니다.
- retry key는 `sessionId + questionId + normalized_query + mode` 같은 안정적인 idempotency key를 사용합니다.
- Spring은 같은 idempotency key의 retry 결과를 하나의 AI message로만 저장해야 합니다.
- Python은 `single-flight`와 answer cache를 이용해 같은 미캐시 질문의 중복 생성을 줄입니다.

## 23.3 UX Resilience와 Graceful Degradation

운영 안정성은 서버가 죽지 않는 것만으로 충분하지 않습니다. 사용자는 대기 중인지, 느려졌는지, 대체 답변으로 전환됐는지를 알아야 합니다.

✅ **권장 UX 문구 흐름**

| 상태 | 사용자 메시지 | 목적 |
|---|---|---|
| LOADING | `AI가 답변을 생성 중입니다...` | 정상 생성 시작을 알림 |
| SLOW | `응답이 지연되고 있습니다. 잠시만 기다려 주세요.` | 5~8초 이상 대기 시 불안 완화 |
| RETRYING | `AI 서버 응답이 느려 다시 시도하고 있습니다.` | retry가 사용자에게 투명하게 보이도록 함 |
| FALLBACK | `대체 답변을 준비 중입니다.` | LLM 실패가 곧 장애가 아님을 전달 |
| ERROR | `지금은 AI 답변을 만들 수 없어 기본 복습 안내를 보여드립니다.` | graceful degradation |

✅ **Graceful degradation 전략**

- 1단계: cache hit 또는 lightweight fast-path로 즉시 응답
- 2단계: Python RAG/LLM 생성 시도
- 3단계: timeout/retry 후 template/rule fallback
- 4단계: fallback도 실패하면 사용자가 다음 문제로 이동할 수 있는 UI 제공
- 5단계: 실패 이벤트는 observability에 남기되, 사용자의 학습 흐름은 끊지 않습니다.

운영 장애 발생 시 사용자 경험은 아래 흐름을 목표로 합니다.

```mermaid
flowchart TD
    A["사용자 질문 제출"] --> B["LOADING: AI가 답변을 생성 중입니다"]
    B --> C{"5~8초 초과?"}
    C -->|"no"| D["SUCCESS: 답변 표시"]
    C -->|"yes"| E["SLOW: 응답이 지연되고 있습니다"]
    E --> F{"timeout?"}
    F -->|"no"| D
    F -->|"yes"| G["RETRYING: 다시 시도 중"]
    G --> H{"retry 성공?"}
    H -->|"yes"| D
    H -->|"no"| I["FALLBACK: 대체 답변 준비"]
    I --> J["기본 복습 안내 표시"]
    J --> K["다음 문제 / 다시 시도 버튼 제공"]
```

## 23.4 AI Request Lifecycle State Machine

Frontend, Spring, Python이 서로 다른 상태 이름을 쓰면 UX, logging, monitoring이 엇갈립니다. 아래 state machine을 공통 contract로 정의하는 것이 좋습니다.

```mermaid
stateDiagram-v2
    [*] --> IDLE
    IDLE --> LOADING: request submitted
    LOADING --> STREAMING: first token/chunk received
    STREAMING --> SUCCESS: final answer completed
    LOADING --> SUCCESS: non-streaming answer completed
    LOADING --> TIMEOUT: read timeout
    STREAMING --> TIMEOUT: stalled stream
    TIMEOUT --> RETRYING: retryable error
    RETRYING --> LOADING: retry attempt
    RETRYING --> FALLBACK: retry exhausted
    LOADING --> FALLBACK: non-retryable LLM failure
    FALLBACK --> SUCCESS: fallback answer shown
    FALLBACK --> ERROR: fallback failed
    ERROR --> IDLE: user dismisses/retries
    SUCCESS --> IDLE: next interaction
```

### 공통 사용 방식

- **Frontend**: 버튼 disable, loading copy, streaming renderer, retry button, fallback banner를 state에 연결합니다.
- **Spring**: `ai_request_lifecycle_state`, `retry_attempt`, `fallback_reason`, `correlation_id`를 로그/metric에 포함합니다.
- **Python**: `observability_events`에 `state`, `model_used`, `cache_hit`, `llm_call_avoided`, `latency_ms`, `fallback_used`를 함께 남깁니다.
- **Monitoring**: `TIMEOUT`, `RETRYING`, `FALLBACK`, `ERROR` 비율을 대시보드화합니다.

✅ 이 state machine을 쓰면 UX, observability, logging, monitoring이 같은 언어로 이어집니다. 예를 들어 `TIMEOUT → RETRYING → FALLBACK`이 늘어나면 모델 품질 문제가 아니라 serving capacity 또는 timeout 설정 문제로 바로 좁힐 수 있습니다.

## 23.5 Streaming, Cache Invalidation, Hallucination Metric

### Streaming UX 설계

- Python은 별도 `/api/review/follow-up/stream`부터 시작합니다.
- Spring은 SSE(Server-Sent Events)를 1순위로 고려합니다.
- Frontend는 `EventSource` 또는 `fetch + ReadableStream`으로 token chunk를 append합니다.
- partial answer 중 cancel 시 UI는 즉시 중단하되, Ollama generation이 실제로 중단됐는지는 별도 metric으로 확인합니다.
- streaming 중 fallback이 필요하면 partial message를 `interrupted`로 닫고 fallback answer를 별도 final message로 표시합니다.

### Cache invalidation 전략

- answer cache key에는 `mode`, `normalized_query`, `model`, `prompt_version`, `knowledge_index_version`을 포함합니다.
- concept card 승인, reindex, prompt version 변경, model 변경 시 cache namespace를 바꿉니다.
- 잘린 답변이나 hallucination 답변이 cache에 들어간 경우 운영자가 특정 key 또는 namespace를 삭제할 수 있어야 합니다.
- Redis 도입 시 TTL은 짧게 시작합니다. 예: free-question 1~6시간, concept-card fast-path는 card hash 기반으로 장기 캐시.

### Hallucination evaluation metric

LLM 답변 평가는 keyword 포함 여부만으로 부족합니다. 아래 metric을 golden dataset과 production sampling에 같이 둡니다.

| Metric | 설명 | 목표 |
|---|---|---|
| groundedness | RAG/context에 근거한 문장 비율 | 0.85 이상 |
| contradiction rate | context와 모순되는 문장 비율 | 2% 이하 |
| unsupported claim rate | 근거 없이 추가한 기술 주장 비율 | 5% 이하 |
| answer relevance | 질문 topic과 답변 핵심어의 의미 유사도 | 0.8 이상 |
| fallback downgrade rate | LLM UNDERSTOOD를 rule/keyword gate가 PARTIAL로 낮춘 비율 | 추세 관찰 |

🔴 hallucination이 의심되는 답변은 cache에 넣지 않습니다. ✅ `confidence < 0.7`, contradiction detected, Korean ratio failure, topic relevance failure 중 하나라도 있으면 rule/template fallback 또는 PARTIAL downgrade를 우선합니다.
