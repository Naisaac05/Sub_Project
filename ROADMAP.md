...
| Phase 4 | 세션 및 결제 UI | 스케줄/세션 프론트엔드 | 3주 |
...
## 8. Phase 4: 매칭 및 세션 관리 UI (3주)

Google Calendar와 Meet API 연동을 시스템에 강제하는 대신, 자체 플랫폼 시스템과 멘토 자율 링크 입력을 유도하여 F-Lab과 동일한 자체 일정 관리 시스템 환경을 구축합니다.

### 설계 변경에 따른 구현 내용

1. **Google Calendar Stub 처리**: 초기 기획된 구글 캘린더 연동은 의존성과 진입 장벽 문제가 있으므로, Stub 상태를 유지하고 내부 Session 서비스 로직을 우선합니다.
2. **화상 회의 링크 멘토 직접 입력**: 멘토가 Zoom, Discord, Google Meet 등 각자의 도구로 생성한 링크를 `MentoringSession.meetLink` 필드에 수동으로 입력하도록 합니다.
3. **프론트엔드 연동 완성**:
    - 매칭 신청서 작성 및 수강 결제 UI (토스 위젯)
    - 성향 조사 및 자체 알고리즘에 기반한 멘토 추천 리스트 UI
    - 멘토 스케줄 가용 시간 설정 UI 및 멘토-멘티 스케줄 교차 검증 로직 연결
    - 확정된 멘토링 세션 리스트와 회의 링크 확인 페이지

### 주요 흐름

사용자는 결제 전 `Application` 폼을 작성하여 '즉시 시작' 혹은 '얼리버드'를 선택하고 할인 금액을 확인한 뒤 결제합니다. 결제 성공 시 세밀한 성향 조사(`SurveyResponse`)를 작성하며, 이를 기반으로 스케줄 교차 검사를 포함하여 최적의 멘토를 추천받고 선택하게 됩니다. 멘토는 할당받은 멘티의 확정 세션에 대해 수동으로 화상 회의 링크를 제공합니다.
...
7. Phase II 배포 시 `admin_audit_log` 테이블 생성 DDL 수동 실행 (prod 는 `ddl-auto: validate` 이므로 자동 생성되지 않음)
...
9. Phase II Feature 2 (결제 관리) 배포 시 다음 SQL 수동 실행
...
10. Phase II Feature 3 (게시물 관리) 배포 시 다음 SQL 수동 실행
